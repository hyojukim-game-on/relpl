...
class DistanceRelayingFragment: RelayingFragment() {
                ...
                relayingViewModel.trackingStateFlow.collect{ list ->
                    ...
                    if(list.isNotEmpty()){
                        naverMap?.let { map ->
                            ...
                            map.moveCamera(CameraUpdate.scrollTo(list.last().latLng).animate(CameraAnimation.Linear))
                        }
                        if (list.size >= 2){
    ...
    override fun startRelay() {
        _activity.startForegroundService(Intent(_activity,DistanceLocationTrackingService::class.java))
    }
    ...
    override val mapReady = OnMapReadyCallback{ map ->
        ...
        naverMap!!.run {
            ...
            addOnCameraIdleListener {
                moveCamera(CameraUpdate.scrollTo(locationOverlay.position).animate(CameraAnimation.Linear))
            }
        }
